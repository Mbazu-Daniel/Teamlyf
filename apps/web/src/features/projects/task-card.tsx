...
export function TaskCard({ task, statuses, onMove, onSelect, onDelete, onDuplicate }: {
  ...
  return (
    <article draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", task.id); }} className="group cursor-grab rounded-md border border-border bg-card p-3 shadow-sm transition hover:border-primary/30 hover:shadow-md active:cursor-grabbing">
      ...
      <select value={task.statusId} onChange={(event) => onMove(task, event.target.value)} onClick={(event) => event.stopPropagation()} className="mt-3 w-full rounded-lg border bg-background px-2.5 py-1.5 text-xs" aria-label={`Status for ${task.name}`}>
