  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-background">
      <div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-3 border-b bg-background/95 px-5 py-3 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          <div className="flex items-center rounded-lg bg-secondary p-1">
            <button type="button" onClick={() => setViewMode("board")} className={`inline-flex h-8 items-center gap-2 rounded-md px-3 text-xs font-medium ${viewMode === "board" ? "bg-background text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><IconLayoutGrid className="size-3.5" /> Board</button>
            <button type="button" onClick={() => setViewMode("list")} className={`inline-flex h-8 items-center gap-2 rounded-md px-3 text-xs font-medium ${viewMode === "list" ? "bg-background text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><IconList className="size-3.5" /> List</button>
          </div>
          ...
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-9 w-40 rounded-md border bg-background px-3 text-xs"><option value="all">All projects</option>{STATUS_OPTIONS.slice(1).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
        </div>
        <div className="flex items-center gap-2">
          <Link to="/$organizationSlug" params={{ organizationSlug }} className="hidden items-center gap-1.5 rounded-md border px-3 py-2 text-xs font-medium hover:bg-muted sm:inline-flex"><IconArrowLeft className="size-3.5" /> Home</Link>
          <Link to="/$organizationSlug/settings" params={{ organizationSlug }} className="hidden items-center gap-1.5 rounded-md border px-3 py-2 text-xs font-medium hover:bg-muted sm:inline-flex"><IconSettings className="size-3.5" /> Settings</Link>
          <button type="button" onClick={() => setShowCreateProject((value) => !value)} className="inline-flex h-9 items-center gap-1.5 rounded-md bg-primary px-4 text-xs font-semibold text-primary-foreground shadow-sm hover:bg-primary/90"><IconPlus className="size-4" /> {showCreateProject ? "Close" : "Add Project"}</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5">
        {state.projectsLoading ? <div className="flex h-40 items-center justify-center"><MutedMessage message="Fetching projects..." /></div> : filteredProjects.length === 0 ? (
          <div className="flex min-h-[420px] items-center justify-center text-center"><div><div className="mx-auto mb-6 flex size-16 items-center justify-center rounded-2xl border border-border/50 bg-primary/10"><IconLayoutGrid className="size-8 text-primary/70" /></div><h3 className="mb-5 text-lg font-bold">{state.projects.length === 0 ? "Create your first project" : "No projects found"}</h3><button type="button" onClick={() => setShowCreateProject(true)} className="rounded-md bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground"><IconPlus className="mr-2 inline size-4" /> Create project</button></div></div>
        ) : viewMode === "board" ? (
