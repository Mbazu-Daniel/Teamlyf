...
export function DashboardHero({organizationSlug,organizationName,projectCount,activeCount}:{organizationSlug:string;organizationName:string;projectCount:number;activeCount:number}) {
 return <section className="flex flex-col gap-5">
  ...
  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{[["Projects",projectCount],["Active",activeCount],["Completed",0],["Planned",0]].map(([label,value])=><div key={label as string} className="rounded-[12px] border border-border/60 bg-white dark:bg-card p-5"><p className="text-3xl font-semibold tabular-nums">{value}</p><p className="mt-3 text-xs text-muted-foreground">{label}</p></div>)}</div>
 </section>;
}