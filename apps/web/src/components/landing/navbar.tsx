    ...
    <header className="landing-nav fixed inset-x-0 top-0 z-30">
      <nav aria-label="Primary" className="mx-auto flex h-[72px] max-w-6xl items-center px-4 sm:px-6">
        <Link to="/" className="flex items-center gap-2.5" aria-label="Teamlyf home">
          <span className="grid size-8 place-items-center rounded-[10px] bg-[var(--landing-ink)] text-sm font-bold text-white">T</span>
        ...
        <div className="ml-8 hidden items-center gap-1 md:flex">
          {links.map((link) => (
            <a key={link.href} href={link.href} className="rounded-full px-3.5 py-2 text-sm font-medium text-[var(--landing-muted)] transition-colors hover:bg-white/70 hover:text-[var(--landing-ink)]">{link.label}</a>
          ))}
        </div>
